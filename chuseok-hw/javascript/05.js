...
function sortStudentsByScore(arr) {
  const newArr = [...arr].sort((a, b) => b.score - a.score);
  return newArr;
}
...
function areAllNumbersGreaterThan(arr, n) {
  for (const e of arr) {
    if (e < n) {
      return false;
    }
...
function getProductsByCategory(arr, name) {
  let newArr = arr.filter((product) => {
