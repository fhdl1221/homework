...
// 태그는 요소의 기능을 나타내고, 시작태그와 종료태그로 구성된다. 종료태그가 없는 태그도 있다.
// 속성은 요소의 추가 특성을 나타내고, 시작태그 내부에 작성한다.
// 컨텐츠는 웹 페이지에 표시할 내용이며 태그 사이에 작성한다.

// 문제 [개념]
// HTML 속성 중 class 속성의 역할에 대해 작성한다
// class 속성은 여러 개의 요소를 그룹화하기 위해 사용한다. 하나의 요소에 여러 개의 클래스를 지정할 수 있고, 하나의 클래스를 여러 개의 요소에 지정할 수 있다.

// 문제 [개념]
// JavaScript 원시 자료형 중 기본 자료형 3개에 대해 작성한다
// 숫자형, 문자열, 불리언이 기본 자료형이다. 숫자형은 모든 숫자를 나타낸다. 문자열은 큰 따옴표, 작은 따옴표, 백틱으로 나타낸다. 불리언은 참, 거짓을 나타낸다.
...
// >는 초과, <는 미만, >=는 이상, <=는 이하를 나타낸다.
// ===는 동등 비교에서 같다를 나타내고, !==는 동등비교에서 다름을 나타낸다
...
// 배열은 대괄호를 사용하여 생성하고 요소는 쉼표를 사용하여 구분한다.
// 배열의 요소에는 대괄호와 인덱스를 사용하여 접근한다.
// push(x)를 사용하여 배열의 끝에 요소를 추가할 수 있다.
// pop()을 사용하여 배열의 마지막 요소를 삭제할 수 있다.
...
// 객체는 중괄호를 사용하여 생성하고, key와 value는 콜론을 통해 구분한다.
// 마침표나 대괄호를 사용하여 객체 속성에 접근한다
// 마침표나 대괄호를 사용하여 객체 속성을 추가한다
// delete 객체명.속성명; 을 사용하여 객체 속성을 삭제한다
...
// map()은 콜백 함수의 반환값을 요소로 하는 새로운 배열을 생성한다
// filter()는 콜백 함수의 반환값이 true인 요소들을 요소로 하는 새로운 배열을 생성한다

// 문제 [개념]
// 배열의 고차 메서드 map()과 filter()의 차이점을 작성한다
// map()은 모든 요소를 새로운 배열에 포함하지만 filter()는 조건에 맞는 요소만 새로운 배열에 포함한다
...
let newNumbers = numbers3.map((number) => {
...
let object8 = { ...object7, country: "대한민국" };
...
let laptopWithWarranty = { ...laptop, warranty: "2년" };
...
// 배열이나 객체 안에 있는 요소를 꺼내 변수에 담는 문법이다
...
const caryear = ({ brand: carBrand, model: carModel, year: carYear } = car2);
